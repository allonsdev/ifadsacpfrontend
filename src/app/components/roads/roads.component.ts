...
export class RoadsComponent implements OnInit {
  ...
  public getRegister() {
    const apiEndpoint = apiUrl + '/Road/detailed';
  ...
  private initializeDataTable(data: any[]) {
    ...
    const editButtonColumn = [{ data: "action", defaultContent: '' }, {
      data: "action",
      defaultContent: '<button class="btn btn-sm btn-icon-only btn-outline-secondary mb-0 btn-sm d-flex align-items-center justify-content-center px-3" data-bs-toggle="modal" data-bs-target="#roadsmodal"><i class="fa fa-pencil" aria-hidden="true"></i></button>',
      title: "",
    ...
    const updatedColumns = [...editButtonColumn, ...columns, {
      ...
      createdCell: (cell: any, cellData: any, rowData: any, rowIndex: number, colIndex: number) => {
        $(cell).on('click', () => {
          this.http.delete(apiUrl + '/Road/' + rowData.Id)
            .subscribe(
              (response: any) => {
                this.getRegister();
                Swal.fire({
    ...
    this.dataTable = $('#dtRoad').DataTable(dtOptions);
