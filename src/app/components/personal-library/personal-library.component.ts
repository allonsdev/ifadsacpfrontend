...
export class PersonalLibraryComponent implements OnInit {
  ...
  public getRegister() {
    this.http
      .get(`${apiUrl}/File/UserCreated/${this.currentUser.staffId}`)
      .subscribe(
        (data: any) => {
          if (!this.dataTable) {
            setTimeout(() => {
  ...
  private initializeDataTable(data: any[]) {
    ...
    const editButtonColumn = [
      {
        data: 'action',
        defaultContent:
          '<button class="btn btn-sm btn-icon-only btn-outline-secondary mb-0 btn-sm d-flex align-items-center justify-content-center px-3" data-bs-toggle="modal" data-bs-target="#filedetailsmodal"><i class="fa fa-pencil" aria-hidden="true"></i></button>',
        title: '',
        createdCell: (
          cell: any,
          cellData: any,
    ...
    this.dataTable = $('#dtUserFiles').DataTable(dtOptions);
