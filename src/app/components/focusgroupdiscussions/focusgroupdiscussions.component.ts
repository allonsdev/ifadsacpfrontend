...
@Component({
  selector: 'app-focusgroupdiscussions',
  templateUrl: './focusgroupdiscussions.component.html'
})
export class FocusgroupdiscussionsComponent implements OnInit {
  currentUser: any;
  dataTable: any;
  id: any;
  ...
  public getRegister() {
    const apiEndpoint = apiUrl + '/FocusGroupDiscussion';
  ...
  private initializeDataTable(data: any[]) {
    ...
    const editButtonColumn = [{ data: "action", defaultContent: '' }, {
      data: "action",
      defaultContent: '<button class="btn btn-sm btn-outline-secondary mb-0 btn-sm d-flex align-items-center justify-content-center px-3" data-bs-toggle="modal" data-bs-target="#focusgroupattachmentsmodal"><i class="fa fa-paperclip" aria-hidden="true"></i></button>',
      title: "",
    ...
    this.dataTable = $('#dtFocusGroupDiscussion').DataTable(dtOptions);
