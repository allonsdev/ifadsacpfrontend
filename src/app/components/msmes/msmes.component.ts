...
@Component({
  selector: 'msmes',
  templateUrl: './msmes.component.html'
})
export class MsmesComponent implements OnInit {
  currentUser: any;
  dataTable: any;
  id: number = 0;
  ...
  private initializeDataTable(data: any[]) {
    ...
    const updatedColumns = [...editButtonColumn, ...columns, {
      ...
      createdCell: (cell: any, cellData: any, rowData: any, rowIndex: number, colIndex: number) => {
        $(cell).on('click', () => {
          this.http.delete(apiUrl + '/MSME/' + rowData.id)
            .subscribe(
              (response: any) => {
                this.getRegister();
                Swal.fire({
    ...
    this.dataTable = $('#dtMSMEs').DataTable(dtOptions);
