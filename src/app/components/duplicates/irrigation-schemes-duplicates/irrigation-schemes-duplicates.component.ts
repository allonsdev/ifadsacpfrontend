...
@Component({
  selector: 'app-irrigation-schemes-duplicates',
  templateUrl: './irrigation-schemes-duplicates.component.html',
})
export class IrrigationSchemesDuplicatesComponent implements OnInit {
  currentUser: any;
  dataTable: any;
  selectedIds: string[] = [];
  ...
  public getDuplicates() {
    const apiEndpoint = apiUrl + '/api/Duplicates/irrigation-schemes';
  ...
  public bulkDelete() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http
          .delete(apiUrl + '/api/Duplicates/irrigation-schemes/bulk', { body: this.selectedIds })
          .subscribe(
            () => {
              this.selectedIds = [];
              this.getDuplicates();
  ...
  private initializeDataTable(data: any[]) {
    const columns: any[] =
      data.length > 0
        ...
        : [{ data: 'HouseholdIdentifierNumber', title: 'HouseholdIdentifierNumber', defaultContent: '-' }];
    ...
    const checkboxColumn = {
      ...
      createdCell: (cell: any, cellData: any, rowData: any) => {
        const id = rowData?.HouseholdIdentifierNumber ?? rowData?.householdIdentifierNumber;
      ...
      createdCell: (cell: any, cellData: any, rowData: any) => {
        ...
        btn.addEventListener('click', () => {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              this.http.delete(apiUrl + '/api/Duplicates/irrigation-schemes/' + id).subscribe(
                () => {
                  this.getDuplicates();
    ...
    this.dataTable = $('#dtIrrigationSchemesDuplicates').DataTable(dtOptions);
