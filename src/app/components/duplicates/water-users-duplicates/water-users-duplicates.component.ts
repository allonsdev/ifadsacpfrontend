...
@Component({
  selector: 'app-water-users-duplicates',
  templateUrl: './water-users-duplicates.component.html',
})
export class WaterUsersDuplicatesComponent implements OnInit {
  currentUser: any;
  dataTable: any;
  selectedIds: number[] = [];
  ...
  public getDuplicates() {
    const apiEndpoint = apiUrl + '/api/Duplicates/water-users';
  ...
  public bulkDelete() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http
          .delete(apiUrl + '/api/Duplicates/water-users/bulk', { body: this.selectedIds })
          .subscribe(
            () => {
              this.selectedIds = [];
              this.getDuplicates();
  ...
  private initializeDataTable(data: any[]) {
      ...
      createdCell: (cell: any, cellData: any, rowData: any) => {
        ...
        btn.addEventListener('click', () => {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              this.http.delete(apiUrl + '/api/Duplicates/water-users/' + id).subscribe(
                () => {
                  this.getDuplicates();
    ...
    this.dataTable = $('#dtWaterUsersDuplicates').DataTable(dtOptions);
