...
@Component({
  selector: 'app-beneficiaryduplicates',
  templateUrl: './beneficiaryduplicates.component.html',
})
export class BeneficiaryduplicatesComponent implements OnInit {
  currentUser: any;
  dataTable: any;
  selectedIds: number[] = [];
  ...
  public getDuplicates() {
    const apiEndpoint = apiUrl + '/api/Duplicates/beneficiary-v3';
  ...
  public bulkDelete() {
    // Read selected IDs directly from checked checkboxes at time of click
    this.selectedIds = [];
    $('.row-checkbox:checked').each((_: any, el: any) => {
      const id = parseInt(el.value);
      if (!isNaN(id)) this.selectedIds.push(id);
    });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http
          .delete(apiUrl + '/api/Duplicates/beneficiary-v3/bulk', { body: this.selectedIds })
          .subscribe(
            () => {
              this.selectedIds = [];
              this.getDuplicates();
  ...
  private initializeDataTable(data: any[]) {
      ...
      createdCell: (cell: any, cellData: any, rowData: any) => {
        ...
        btn.addEventListener('click', () => {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              this.http.delete(apiUrl + '/api/Duplicates/beneficiary-v3/' + id).subscribe(
                () => {
                  this.getDuplicates();
    ...
    const dtOptions: any = {
      ...
      pageLength: 10,
      columns: [checkboxColumn, deleteColumn, ...columns],
      dom: 'BfrtipP',
    ...
    this.dataTable = $('#dtBeneficiaryDuplicates').DataTable(dtOptions);
