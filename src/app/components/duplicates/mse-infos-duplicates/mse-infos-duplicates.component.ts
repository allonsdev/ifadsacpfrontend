...
@Component({
  selector: 'app-mse-infos-duplicates',
  templateUrl: './mse-infos-duplicates.component.html',
})
export class MseInfosDuplicatesComponent implements OnInit {
  currentUser: any;
  dataTable: any;
  selectedIds: number[] = [];
  ...
  public getDuplicates() {
    const apiEndpoint = apiUrl + '/api/Duplicates/mse-infos';
  ...
  public bulkDelete() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http
          .delete(apiUrl + '/api/Duplicates/mse-infos/bulk', { body: this.selectedIds })
          .subscribe(
            () => {
              this.selectedIds = [];
              this.getDuplicates();
  ...
  private initializeDataTable(data: any[]) {
      ...
      createdCell: (cell: any, cellData: any, rowData: any) => {
        ...
        btn.addEventListener('click', () => {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              this.http.delete(apiUrl + '/api/Duplicates/mse-infos/' + id).subscribe(
                () => {
                  this.getDuplicates();
    ...
    this.dataTable = $('#dtMseInfosDuplicates').DataTable(dtOptions);
