...
@Component({
  selector: 'app-road-users-duplicates',
  templateUrl: './road-users-duplicates.component.html',
})
export class RoadUsersDuplicatesComponent implements OnInit {
  currentUser: any;
  dataTable: any;
  selectedIds: number[] = [];
  ...
  public getDuplicates() {
    const apiEndpoint = apiUrl + '/api/Duplicates/road-users';
  ...
  public bulkDelete() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http
          .delete(apiUrl + '/api/Duplicates/road-users/bulk', { body: this.selectedIds })
          .subscribe(
            () => {
              this.selectedIds = [];
              this.getDuplicates();
  ...
  private initializeDataTable(data: any[]) {
      ...
      createdCell: (cell: any, cellData: any, rowData: any) => {
        ...
        btn.addEventListener('click', () => {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              this.http.delete(apiUrl + '/api/Duplicates/road-users/' + id).subscribe(
                () => {
                  this.getDuplicates();
    ...
    this.dataTable = $('#dtRoadUsersDuplicates').DataTable(dtOptions);
